...
<h2 class="mb-4">Barangay Cantil-E</h2>
    <p class="text-muted mb-4">Archive Resident</p>
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4>Archived Residents</h4>
        <a href="{{ route('secretary.residence.all') }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left me-1"></i>Back to All Residents
        </a>
    </div>
    <div class="card-body">
                ...
                <tbody>
                    @forelse($archived_residents as $resident)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($resident->archived_at)->format('M d, Y') }}</td>
                            <td>
                                <div class="btn-group" role="group">
                                    <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#residentModal{{ $resident->id }}">
                                        <i class="fas fa-eye me-1"></i>View
                                    </button>
                                    <form action="{{ route('secretary.residence.restore', $resident->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-success restore-btn" data-id="{{ $resident->id }}">
